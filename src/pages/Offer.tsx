...
function OfferDetailsPage() {
  ...
  const offer = {
    ...
    description: "Get a large pepperoni pizza for a special discounted price! Limited time offer.",
    originalPrice: 18.50,
  ...
  const distance = offer.latitude && offer.longitude
    ? calculateDistance(userLatitude, userLongitude, offer.latitude, offer.longitude)
    : 'N/A';
        ...
        <div className="modern-offer-info">
          ...
          {offer.latitude && offer.longitude && (
            <p className="modern-offer-location">
              <svg viewBox="0 0 24 24" fill="currentColor" className="icon">
                <path fillRule="evenodd" d="M12 1.5a5.25 5.25 0 00-5.25 5.25c0 2.891 2.79 6.324 5.25 8.406 2.459-2.082 5.25-5.515 5.25-8.406A5.25 5.25 0 0012 1.5zM12 8.25a1.5 1.5 0 110-3 1.5 1.5 0 010 3z" clipRule="evenodd" />
              </svg>
              Location: {offer.address || `${offer.latitude}, ${offer.longitude}`}
            </p>
          )}
          ...
          <div className="modern-offer-pricing">
            <span className="modern-original-price">€{offer.originalPrice}</span>
            <span className="modern-discounted-price">€{offer.discountedPrice}</span>
          </div>
